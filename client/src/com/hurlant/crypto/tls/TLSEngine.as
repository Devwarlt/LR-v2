...
package com.hurlant.crypto.tls {
	...
	public class TLSEngine extends EventDispatcher {
		...
		function TLSEngine(config:TLSConfig, iStream:IDataInput, oStream:IDataOutput, otherIdentity:String = null) {
			...
			// setting up new security LOEBUILD_166e64f6c3677d0c513901242a3e702d needs to be controlled by...something.
		...
		private function parseHandshake(p:ByteArray):ByteArray {
			...
			// Surf the handler map and find the right handler for this handshake packet MsgType.
			// I modified the individual handlers so they encapsulate all possible knowledge 
			// about the incoming packet MsgType, so no previous handling or massaging of the data
			...
			if (_entityHandshakeHandlers.hasOwnProperty( type )) {
				...
				throw new TLSError( "Unimplemented or unknown handshake MsgType!", TLSError.internal_error );
			}
		...
		/**
		 * Throw an error when the detected handshake state isn't a valid state for the given entity MsgType (client vs. server, etc. ).
		 * This really should abort the handshake, since there's no case in which a server should EVER be confused about the MsgType of entity it is. BP
		 */
		private function notifyStateError( rec:ByteArray ) : void {
			throw new TLSError( "Invalid handshake state for a TLS Entity MsgType of " + _entity, TLSError.internal_error );
		}
		...
		private function loadCertificates( rec:ByteArray ):void {
			...
			// Test first for trust override LOEBUILD_166e64f6c3677d0c513901242a3e702d
