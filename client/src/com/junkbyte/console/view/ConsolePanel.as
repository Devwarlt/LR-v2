...
public class ConsolePanel extends Sprite {
      ...
      private function updateScale(param1:Event = null) : void {
         ...
         this.width = _local2.x < this.minWidth?Number(this.minWidth):Number(_local2.x);
         this.height = _local2.y < this.minHeight?Number(this.minHeight):Number(_local2.y);
         this.updateScaleText();
      }
      ...
      private function getSnapOf(param1:Number, param2:Boolean) : Number {
         ...
         var _local3:Number = param1 + this.width;
         var _local4:Array = this._snaps[!!param2?0:1];
         var _local5:int = this.style.panelSnapping;
         for each(_local6 in _local4) {
            if(Math.abs(_local6 - param1) < _local5) {
               return _local6;
            }
