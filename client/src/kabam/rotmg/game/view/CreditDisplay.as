...
public class CreditDisplay extends Sprite {
      ...
      public function CreditDisplay(param1:GameSprite = null, param2:Boolean = true, param3:Boolean = false, param4:Number = 0) {
         ...
         if(Boolean(this.displayFortune_) && Boolean(FortuneModel.HAS_FORTUNES)) {
      ...
      private function onDoubleClick(param1:MouseEvent) : void {
         if(!this.gs || Boolean(this.gs.evalIsNotInCombatMapArea()) || LOEBUILD_3225a10b07f1580f10dee4abc3779e6c.data_.clickForGold == true) {
            this.openAccountDialog.dispatch();
         }
      }

      public function makeTextField(param1:uint = 16777215) : TextFieldDisplayConcrete {
         var _local2:TextFieldDisplayConcrete = new TextFieldDisplayConcrete().setSize(FONT_SIZE).setColor(param1).setTextHeight(16);
      ...
      public function draw(param1:int, param2:int, param3:int = 0) : void {
         ...
         if(param1 == this.credits_ && (Boolean(this.displayFame_) && param2 == this.fame_) && (Boolean(this.displayFortune_) && param3 == this.fortune_)) {
            return;
         }
      ...
      public function getFortuneTimeLeftStr() : String {
         ...
         } else if(_local3 > TimeUtil.MIN_IN_S) {
            _local1 = String(Math.ceil(TimeUtil.secondsToMins(_local3))) + " minutes";
         } else if(_local3 > TimeUtil.MIN_IN_S / 2) {
