...
namespace wServer.logic.behaviors.PetBehaviors
{
    internal class PetHealMP : Behavior
    {
                ...
                if (player != null)
                {
                    ...
                    if (newMp != player.Mp)
                    {
                        ...
                        if (player.HasConditionEffect(ConditionEffects.Quiet))
                        {
                            ...
                            {
                                ObjectId = player.Id,
                                Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"Pet can't heal your MP!\"}}",
                        ...
                        {
                            ObjectId = player.Id,
                            Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"+ " + n + " MP [" + player.Mp + "]\"}}",
                            Color = new ARGB(0x6084e0)
                        }, null);
                    }
                }
            ...
            for (int i = 0; i < 3; i++)
            {
                switch (i)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        if ((host.SecondPetLevel.Ability == Ability.MagicHeal) && (host.PetRarity > Rarity.Common))
                        {
                            return CalculateMagicHeal(host.SecondPetLevel.Level, ref cooldown);
                        }
                        break;
                    case 2:
                        if ((host.ThirdPetLevel.Ability == Ability.MagicHeal) && (host.PetRarity > Rarity.Rare))
                        {
        ...
        private int CalculateMagicHeal(int level, ref int cooldown)
        {
            ...
            else if (Enumerable.Range(101, 1).Contains(level)) { cooldown = 1000; return 44; }
            else if (Enumerable.Range(102, 1).Contains(level)) { cooldown = 1000; return 45; }
            else if (Enumerable.Range(103, 1).Contains(level)) { cooldown = 1000; return 46; }
            else if (Enumerable.Range(104, 1).Contains(level)) { cooldown = 1000; return 47; }
            else if (Enumerable.Range(105, 1).Contains(level)) { cooldown = 1000; return 48; }
            else if (Enumerable.Range(106, 1).Contains(level)) { cooldown = 1000; return 49; }
            else if (Enumerable.Range(107, 1).Contains(level)) { cooldown = 1000; return 50; }
            else if (Enumerable.Range(108, 1).Contains(level)) { cooldown = 1000; return 51; }
            else if (Enumerable.Range(109, 1).Contains(level)) { cooldown = 1000; return 53; }
            else if (Enumerable.Range(110, 1).Contains(level)) { cooldown = 1000; return 55; }
            else {
                cooldown = 10000;
                return 1;
            }
