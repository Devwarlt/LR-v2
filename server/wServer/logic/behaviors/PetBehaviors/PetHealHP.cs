...
namespace wServer.logic.behaviors.PetBehaviors
{
    internal class PetHealHP : Behavior
    {
                ...
                if (player != null)
                {
                    ...
                    if (newHp != player.HP)
                    {
                        if (player.HasConditionEffect(ConditionEffects.Sick))
                        {
                            ...
                            {
                                ObjectId = player.Id,
                                Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"Pet can't heal your HP!\"}}",
                                Color = new ARGB(0xFF0000)
                            }, null);
                            state = cool;
                            return;
                        }
                        if(player.Owner.StaticObjects.Where(i => i.Value.ObjectType == 0x015a).Count(i => (player.X - i.Value.X) * (player.X - i.Value.X) + (player.Y - i.Value.Y) * (player.Y - i.Value.Y) < 1) > 0) {
                            player.Owner.BroadcastPacket(new ShowEffectPacket
                            {
                                EffectType = EffectType.Trail,
                                TargetId = host.Id,
                            ...
                            {
                                ObjectId = player.Id,
                                Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"I'm burning!\"}}",
                                Color = new ARGB(0xFF0000)
                            }, null);
                            state = cool;
                            return;
                        ...
                        player.Owner.BroadcastPacket(new ShowEffectPacket
                        {
                            EffectType = EffectType.Potion, //potion
                            TargetId = player.Id,
                            Color = new ARGB(0xffffffff)
                        }, null);
                        player.Owner.BroadcastPacket(new ShowEffectPacket
                        ...
                        {
                            ObjectId = player.Id,
                            Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"+ " + n + " HP [" + player.HP + "]\"}}",
            ...
            for (int i = 0; i < 3; i++)
            {
                switch (i)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        if ((host.SecondPetLevel.Ability == Ability.Heal) && (host.PetRarity > Rarity.Common))
                        {
                            return CalculateHeal(host.SecondPetLevel.Level, ref cooldown);
                        }
                        break;
                    case 2:
                        if ((host.ThirdPetLevel.Ability == Ability.Heal) && (host.PetRarity > Rarity.Rare))
                        {
        ...
        {
            if (Enumerable.Range(1, 1).Contains(level)) { cooldown = 10000; return 10; }
            else if (Enumerable.Range(2, 1).Contains(level)) { cooldown = 9350; return 10; }
            else if (Enumerable.Range(3, 1).Contains(level)) { cooldown = 8690; return 10; }
            else if (Enumerable.Range(4, 1).Contains(level)) { cooldown = 8040; return 10; }
            else if (Enumerable.Range(5, 1).Contains(level)) { cooldown = 7390; return 10; }
            else if (Enumerable.Range(6, 1).Contains(level)) { cooldown = 7130; return 10; }
            else if (Enumerable.Range(7, 1).Contains(level)) { cooldown = 6880; return 10; }
            else if (Enumerable.Range(8, 1).Contains(level)) { cooldown = 6620; return 10; }
            ...
            else if (Enumerable.Range(101, 1).Contains(level)) { cooldown = 1000; return 79; }
            else if (Enumerable.Range(102, 1).Contains(level)) { cooldown = 1000; return 81; }
            else if (Enumerable.Range(103, 1).Contains(level)) { cooldown = 1000; return 83; }
            else if (Enumerable.Range(104, 1).Contains(level)) { cooldown = 1000; return 85; }
            else if (Enumerable.Range(105, 1).Contains(level)) { cooldown = 1000; return 87; }
            else if (Enumerable.Range(106, 1).Contains(level)) { cooldown = 1000; return 90; }
            else if (Enumerable.Range(107, 1).Contains(level)) { cooldown = 1000; return 93; }
            else if (Enumerable.Range(108, 1).Contains(level)) { cooldown = 1000; return 96; }
            else if (Enumerable.Range(109, 1).Contains(level)) { cooldown = 1000; return 99; }
            else if (Enumerable.Range(110, 1).Contains(level)) { cooldown = 1000; return 100; }
            else
            {
                cooldown = 10000;
                return 10;
