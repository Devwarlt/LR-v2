...
namespace wServer.realm
{
    public class LogicTicker
    {
        ...
        public int MsPT;
        public int TPS;
        public const int PENDING_ACTIONS = 7;

        public LogicTicker(RealmManager manager)
        {
            Manager = manager;
            pendings = new ConcurrentQueue<Action<RealmTime>>[PENDING_ACTIONS];
            for (int i = 0; i < PENDING_ACTIONS; i++)
...
namespace wServer.realm
{
    public class LogicTicker
    {
        ...
        {
            Manager = manager;
            pendings = new ConcurrentQueue<Action<RealmTime>>[6];
            for (int i = 0; i < 6; i++)
                pendings[i] = new ConcurrentQueue<Action<RealmTime>>();

            TPS = manager.TPS;
            MsPT = 1000/TPS;//normal 1000 / 1300
        }
        ...
        public void TickLoop()
        {
            ...
            do
            {
                ...
                count += times;
                //if (times > 3)
                //    log.Warn("LAGGED!| time:" + times + " dt:" + dt + " count:" + count + " real time:" + b + " tps:" +
                //             count/(b/1000.0));
                ticked = false;
                if(ticked == true)
                {
                    ...
                    t.tickCount = count;
                    t.thisTickCounts = (int) times;
                    t.thisTickTimes = (int) times*MsPT*105/100;
                } else
                {
                    t.tickTimes = b;
                    t.tickCount = count;
